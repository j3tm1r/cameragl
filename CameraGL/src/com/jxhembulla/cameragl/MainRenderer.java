...
public class MainRenderer implements GLSurfaceView.Renderer,
    ...
    private static synchronized void setCameraParameters(Camera.Parameters params) {
        if (mCamera != null) {
    ...
    public void onSurfaceCreated(GL10 unused, EGLConfig config) {
        ...
        if (psize.size() > 0) {
            ...
            int bufferSize = psize.get(i).height * psize.get(i).width * bitsPerPixel / 8;
            callbackBuffer = null;
    ...
    public void touchEvent(float x, float y) {
        ...
        int offset = previewSize.width * previewSize.height;

        int divideby = 0;
        int y1, y2, y3, y4;
        int r, g, b;

        int pixelh = (int)(((float)previewSize.height) * y);
        int pixelw = (int)(((float)previewSize.width) * x);
        ...
        Log.d("CameraRenderer", "R for pixel " + r + "G for pixel " + g + " B for pixel " + b);
